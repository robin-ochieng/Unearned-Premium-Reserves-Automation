...
ui <- fluidPage(
  ...
  tags$div(id = "header",
           tags$h1("Unearned Premium Reserving Analysis Tool", style = "display: inline;")
  ),
  sidebarLayout(
    sidebarPanel(
      ...
      tags$div(
        ...
        tags$ul(
          ...
          tags$ul(
            tags$li("BegDate: Representing the policy start date Column."),
            tags$li("EndDate: Representing the policy end date Column."),
            tags$li("IRA CLASS: Representing be the class of Business Column."),
            tags$li("Premium: Representing be the Premium Column."),
          )
        )
      ),
...
server <- function(input, output, session) {
  ...
  output$classWiseUPRPlot <- renderPlot({
    ...
    ggplot(data, aes(x = `IRA Class`, y = Class_wise_UPR_Sum, fill = `IRA Class`)) +
      ...
      geom_text(aes(label = scales::comma(Class_wise_UPR_Sum)),  # Add this line

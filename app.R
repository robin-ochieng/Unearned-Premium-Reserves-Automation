...
server <- function(input, output, session) {
  ...
  summaryData <- eventReactive(input$goButton, {
    ...
    for (yr in 2021:2024) {
      ...
      quarters_to_iterate <- if (yr == 2024) c("Q1") else c("Q1", "Q2", "Q3", "Q4")
      for (quarter in quarters_to_iterate) {
        quarter_EP_col <- paste0(quarter, "_", yr, "_EP")
        data[[quarter_EP_col]] <- calculate_EP(data, year_quarters, quarter, yr)
      }
    }
    data %>%
      group_by(`IRA Class`) %>%
...
calculate_EP <- function(data, year_quarters, quarter, year) {
  quarter_EP <- ifelse(data$Auth_Year < 2013, 0,
